/*
First edition on GPU
*/

__global__ void kernel_cpu_im2col(float *_I, float *workspace, int N, int C, int H, int W,
                 ...
                 int dilation_w)
{
  ...
  // Start kernel call
  kernel_cpu_im2col<<<1, 1>>>(I_GPU, BUF1_GPU, N, C, H, W, R, S, pad_h, pad_w, stride_h, stride_w,
                              dilation_h, dilation_w);
  CHECK_CUDA(cudaGetLastError());
  kernel_cpu_matmul<<<1, 1>>>(F_GPU, BUF1_GPU, BUF2_GPU, K, N * OH * OW, C * R * S);
  CHECK_CUDA(cudaGetLastError());
  kernel_reshape<<<1, 1>>>(BUF2_GPU, O_GPU, N, K, OH, OW);
  CHECK_CUDA(cudaGetLastError());

  CHECK_CUDA(cudaMemcpy(_O, O_GPU, ON * OC * OH * OW * sizeof(float), cudaMemcpyDeviceToHost));
                 ...
                 int dilation_w)
{
  ...
  kernel_cpu_matmul<<<grid2, block2>>>(F_GPU, BUF1_GPU, BUF2_GPU, K, N * OH * OW, C * R * S);
  CHECK_CUDA(cudaGetLastError());
                 ...
                 int dilation_w)
{
  ...
  kernel_cpu_im2col<<<grid1, block1>>>(I_GPU, BUF1_GPU, N, C, H, W, R, S, pad_h, pad_w, stride_h, stride_w,
                                       dilation_h, dilation_w);
  CHECK_CUDA(cudaGetLastError());
